function results = error_metric_new(pred, gt, mask)
...
rmslog = (log(gt(:)) - log(pred(:))).^2;
rmslog(~mask) = 0;
rmslog = sqrt(sum(rmslog)/n_pxls);
fprintf('Root Mean Squared log scale Error: %4f\n', rmslog);
...
mask = double(ratio<1.25);
acc_1 = mean(mask(:));
mask = double(ratio<1.25^2);
acc_2 = mean(mask(:));
mask = double(ratio<1.25^3);
acc_3 = mean(mask(:));

fprintf('delta smaller than 1.25: %4f\n', acc_1);
fprintf('delta smaller than 1.25^2: %4f\n', acc_2);
fprintf('delta smaller than 1.25^3: %4f\n', acc_3);
